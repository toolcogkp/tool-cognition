...
{
public:
    M3MoveGroup(std::string name):
    group(name),
    rgrp_name("right_arm"),
    group_right("right_arm"),
    lgrp_name("left_arm"),
    group_left("left_arm"),
  ...
  void stop_fingers(std::vector<std::string> names){
     ...
     cout << "finger cmd 2=:" << finger_cmd.values[1] << endl;
      ...
      //cout << names.at(1) << " finger stop command published." << endl;
  }
  ...
  void FingersEncodersActionCB(const m3_moveit::MoveitFingersGoalConstPtr& goal) {
      ...
      if(-(finger_live_right1 - finger_target_right1) >= tol_finger){       //negative
      ...
      if(-(finger_live_right2 - finger_target_right2) >= tol_finger){       //negative
          finger_cmd.values.push_back(open_fingers_currentmode_value);
      }
...
//              if(!group_fingers.move()){
              if(!group_fingers.plan(plan)){ //real robot fingers not set up for encoder feedback
                  result.error_code = result.FAIL;
              }
              else{
 ...
 //------------------------------------------------------------------------------------------------------------------------------
    ...
    void CartSingleRightActionCB(const m3_moveit::MoveitSingleGoalConstPtr &goal)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if(leftFlag)
                {
                  // robot_trajectory::RobotTrajectory rt(group_left.getCurrentState()->getRobotModel(), "left_arm");
  ...
  void Send2OliviaSingleArm(moveit_msgs::RobotTrajectory trajectory, std::string link_name){
      ...
      for(int i=0; i<trajectory.joint_trajectory.points.size(); i++){
        ...
        if(link_name == "l_ee"){
            new_arm_joint_values = ConvertOliviaArm(trajectory.joint_trajectory.points[i].positions, "left");
            for(int j=3; j<10; j++){
                trajectory_whole.joint_trajectory.points[i].positions[j] = new_arm_joint_values[j-3];
//                trajectory_whole.joint_trajectory.points[i].velocities[j] = trajectory.joint_trajectory.points[i].velocities[j-3];
//                trajectory_whole.joint_trajectory.points[i].accelerations[j] = trajectory.joint_trajectory.points[i].accelerations[j-3];
            }
        }
        else if(link_name == "ee"){
            new_arm_joint_values = ConvertOliviaArm(trajectory.joint_trajectory.points[i].positions, "right");
            for(int j=11; j<18; j++){
                trajectory_whole.joint_trajectory.points[i].positions[j] = new_arm_joint_values[j-11];
//                trajectory_whole.joint_trajectory.points[i].velocities[j] = trajectory.joint_trajectory.points[i].velocities[j-11];
//                trajectory_whole.joint_trajectory.points[i].accelerations[j] = trajectory.joint_trajectory.points[i].accelerations[j-11];
            }
        }
...
//          cout << trajectory.joint_trajectory.joint_names.at(i) << endl;
  ...
  //------------------------------------------------------------------------------------------------------------------------------
  //------------------------------------------------------------------------------------------------------------------------------
  //samuel USING THIS RIGHT FUNCTION!
      ...
      for(int i=0; i<goal->end_eff.poses.size(); i++)
      {
          ...
          else 
          {
              ...
              else
              {
                  ...
                  for(int i=0; i<trajectory.joint_trajectory.points.size(); i++){
                    // std::cout << "time_from_start = " << trajectory.joint_trajectory.points[i].time_from_start << std::endl;
  ...
  //------------------------------------------------------------------------------------------------------------------------------
    ...
    {
        ROS_INFO_STREAM("\n\nATTACH OBJ CB");
        // ROS_INFO_STREAM("id: " << group_right.getPlanningFrame());

        m3_moveit::MoveitPickPlaceResult result;
        bool success = false;

        // group.attachObject(goal->object_name, "ee", rightHandLinkNames);
  ...
  void JointWholeBodyActionCB(const m3_moveit::MoveitWholeBodyGoalConstPtr& goal) {
      ...
      moveit::planning_interface::MoveGroup::Plan plan;
        ...
        std::vector<double> group_variable_values;
        //std:cout << "No of joint trajectory points=" << goal->left_trajectory.points.size() << std::endl;
        //group_variable_values.resize(17);
  ...
  //------------------------------------------------------------------------------------------------------------------------------
  //------------------------------------------------------------------------------------------------------------------------------
  //work in progress, same as using wholejoint body using planned move request
  ...
  bool CheckHandOrientation(const moveit::planning_interface::MoveGroup::Plan plan, const geometry_msgs::Pose pose_left, const geometry_msgs::Pose pose_right){
      ...
      for(int j=0;j<plan.trajectory_.joint_trajectory.points.size();j++){
          //for(int k=0;k<plan.trajectory_.joint_trajectory.points[j].positions.size();k++)
              //std::cout << "plan.trajectory_.joint_trajectory.points[j].positions[" << k << "] = " << plan.trajectory_.joint_trajectory.points[j].positions[k] << std::endl;
